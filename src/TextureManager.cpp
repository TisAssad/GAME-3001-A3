...
void TextureManager::Draw(const std::string& id, const glm::vec2 position, GameObject* go, const double angle, const int alpha, const bool centered, const SDL_RendererFlip flip)
{
	SDL_Rect src_rect{};
	SDL_Rect dest_rect{};
	...
	if (centered) {
		...
		dest_rect.x = position.x - x_offset;
		dest_rect.y = position.y - y_offset;
	}
	else {
...
void TextureManager::PlayAnimation(
	...
	const int x, const int y, const float speed_factor, const glm::vec2 scalar,
	const double angle, const int alpha, const bool centered, const SDL_RendererFlip flip)
{
	...
	dest_rect.w = texture_width * scalar.x;
	dest_rect.h = texture_height * scalar.y;
...
{
	PlayAnimation(sprite_sheet_name, animation, static_cast<int>(position.x), static_cast<int>(position.y), speed_factor, { 1,1 }, angle, alpha, centered, flip);
}
