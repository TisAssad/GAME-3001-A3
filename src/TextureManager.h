...
public:
	...
	void Draw(const std::string& id, glm::vec2 position, GameObject* go, double angle = 0, int alpha = 255, bool centered = false, SDL_RendererFlip flip = SDL_FLIP_NONE);
	...
	void PlayAnimation(const std::string& sprite_sheet_name, Animation& animation, int x, int y, float speed_factor, glm::vec2 scalar, double angle, int alpha, bool centered = false, SDL_RendererFlip flip = SDL_FLIP_NONE);
